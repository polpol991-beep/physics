    ...
    if exam_open_flag != '1':
        ...
        if st.session_state.eval_started and st.session_state.eval_status == "진행중":
            ...
            with st.form("eval_form"):
                ...
                # 🖼️ 나중에 그림 추가: st.image("img/q1_spanner.png", use_container_width=True)
                ...
                # 🖼️ 나중에 그림 추가: st.image("img/q2_seesaw.png", use_container_width=True)
                ...
                # 🖼️ 나중에 그림 추가: st.image("img/q3_stick.png", use_container_width=True)
                ...
                # 🖼️ 나중에 그림 추가: st.image("img/q4_bridge.png", use_container_width=True)
                a4 = st.radio("4번 답 선택", ["기둥 A가 받는 힘이 더 크다.", "기둥 B가 받는 힘이 더 크다.", "기둥 A와 B가 받는 힘은 같다."], index=None, key="a4")
                
                st.markdown("**문제 5. [객관식] (15점)** 2021년식 아반떼 차량과 같이 차체가 낮고 바닥 면적이 넓은 승용차와, 짐을 높게 쌓아 올려 차체가 높은 화물차가 동일한 각도의 굽은 경사로를 달리고 있습니다. 두 차량 중 더 안정적인 차량과 그 이유로 올바른 것은?")
                # 🖼️ 나중에 그림 추가: st.image("img/q5_car.png", use_container_width=True)
                ...
                # 🖼️ 나중에 그림 추가: st.image("img/q6_cat.png", use_container_width=True)
