...
result_sheet = spreadsheet.worksheet("돌림힘 수행평가") # 👈 새로운 수행평가 기록 시트
...
<body>
    <div class="instructions">
        ...
        <strong>'시도하기'</strong>를 눌러 수평이 되는지 확인하고, 질량을 예측해 보세요!
    </div>
    ...
    <div class="weight" id="w-a" data-mass="5">시료<br>A</div>
    <div class="weight" id="w-b" data-mass="15">시료<br>B</div>
    <div class="weight" id="w-c" data-mass="20">시료<br>C</div>
...
# =====================================================================
# 🌟 [신규] 회원정보 E1 셀 시험 개방 여부 확인 (10초 캐싱)
# =====================================================================
...
with tab_practice:
    if st.session_state.get('eval_started', False):
        ...
        st.header("🛠️ 돌림힘과 평형 연습하기")
        st.info("💡 자유롭게 추를 옮겨가며 시소가 수평을 이루는 조건을 찾아보세요. 점수에 반영되지 않습니다.")
        components.html(sim_html, height=650, scrolling=False)

# ---------------------------------------------------------
# [탭 2] 수행평가 실시 화면 (E1 통제 + 10점 감점 폭탄)
# ---------------------------------------------------------
with tab_eval:
    ...
    if exam_open_flag != '1':
            ...
            for i, row in enumerate(all_rows):
                if len(row) > 0 and row[0] == s_id:
                    ...
                    if st.session_state.eval_status == "진행중":
                        st.session_state.eval_penalty += 10 # 도망친 경우 10점 감점
        ...
        if start_eval and not st.session_state.eval_started:
            ...
            if st.session_state.eval_status is None:
                ...
                try:
                    # 행 추가: 학번, 이름, 상태, 감점, 맞은개수, 기본점수, 최종점수
        ...
        if st.session_state.eval_started and st.session_state.eval_status == "진행중":
            ...
            components.html(
                ...
                document.addEventListener("visibilitychange", function() {
                    if (document.hidden) { window.parent.alert("🚨 [경고] 화면 이탈이 감지되었습니다! 감점 처리될 수 있습니다."); }
                });
                </script>
                """, height=0
            )
            
            # 시험 볼 때 윗부분에 가상 실험실 띄워주기
            components.html(sim_html, height=650, scrolling=False)
            st.divider()
            
            with st.form("eval_form"):
                st.subheader("📝 수행평가 답안지")
                st.info("위의 시뮬레이션을 활용하여 아래 문제들의 정답을 입력하세요. (총 100점 만점)")
                
                # 1번
                st.markdown("**문제 1. [객관식] (15점)** 자동차 바퀴의 꽉 조여진 볼트를 스패너로 풀려고 합니다. 볼트(회전축)로부터 가까운 A 지점과 먼 B 지점 중, 더 적은 힘으로 볼트를 풀 수 있는 곳은 어디이며 그 이유는 무엇입니까?")
                a1 = st.radio("1번 답 선택", ["A 지점 (회전축에 가까울수록 돌림힘이 커지므로)", "B 지점 (회전축에서 멀수록 작은 힘으로도 큰 돌림힘을 낼 수 있으므로)", "두 지점 모두 필요한 힘은 같다."], index=None, key="a1")
                
                # 2번
                st.markdown("**문제 2. [단답식] (15점)** 무게를 무시할 수 있는 시소의 받침점으로부터 왼쪽으로 2 m 떨어진 곳에 질량이 10 kg인 물체가 놓여 있습니다. 이 시소가 수평을 유지하기 위해 오른쪽으로 5 m 떨어진 곳에 놓아야 할 물체 X의 질량은 몇 kg입니까?")
                a2 = st.number_input("2번 답 입력 (숫자만)", step=1, key="a2")
                
                # 3번
                st.markdown("**문제 3. [단답식] (15점)** 길이가 4 m이고 질량이 2 kg인 균일한 막대가 있습니다. 막대의 왼쪽 끝에서 1 m 떨어진 지점에 받침대를 놓았습니다. 이 막대가 수평을 이루도록 하려면 막대의 왼쪽 끝부분에 몇 kg의 추를 매달아야 합니까?")
                a3 = st.number_input("3번 답 입력 (숫자만)", step=1, key="a3")
                
                # 4번
                st.markdown("**문제 4. [객관식] (15점)** 두 개의 기둥 A(왼쪽)와 B(오른쪽)가 떠받치고 있는 수평 다리 위를 무거운 트럭이 지나가고 있습니다. 트럭이 기둥 B에 더 가깝게 위치해 있을 때, 다리를 떠받치는 힘(수직항력)의 크기에 대한 설명으로 옳은 것은?")
                a4 = st.radio("4번 답 선택", ["기둥 A가 받는 힘이 더 크다.", "기둥 B가 받는 힘이 더 크다.", "기둥 A와 B가 받는 힘은 같다."], index=None, key="a4")
                
                # 5번
                st.markdown("**문제 5. [객관식] (20점)** 2021년식 아반떼 차량과 같이 차체가 낮고 바닥 면적이 넓은 승용차와, 짐을 높게 쌓아 올려 차체가 높은 화물차가 동일한 각도의 굽은 경사로를 달리고 있습니다. 두 차량 중 더 안정적인 차량과 그 이유로 올바른 것은?")
                a5 = st.radio("5번 답 선택", ["승용차 (무게중심이 낮고 지지면이 넓어 안정성이 높다.)", "화물차 (질량이 더 커서 관성에 의해 안정성이 높다.)", "두 차량의 안정성은 동일하다."], index=None, key="a5")
                
                # 6번
                st.markdown("**문제 6. [단답식 - 심화] (20점)** 건물 옥상 밖으로 2 m가 튀어나오도록 널빤지를 놓았습니다. 널빤지 전체의 길이는 6 m이고, 질량은 10 kg으로 균일합니다. 질량이 5 kg인 고양이가 옥상에서 출발하여 밖으로 튀어나온 널빤지 위를 조심스럽게 걸어갑니다. 널빤지가 뒤집어지기 직전까지 고양이는 옥상 모서리(받침점)로부터 최대 몇 m까지 걸어갈 수 있습니까?")
                a6 = st.number_input("6번 답 입력 (숫자만)", step=1, key="a6")
                ...
                if submitted:
                    if None in [a1, a4, a5]:
                        st.error("⚠️ 객관식 문제를 모두 선택해 주세요!")
                    else:
                        # 채점 로직 (문항별 배점 적용)
                        base_score = 0
                        correct_count = 0
                        
                        if a1 == "B 지점 (회전축에서 멀수록 작은 힘으로도 큰 돌림힘을 낼 수 있으므로)": base_score += 15; correct_count += 1
                        if a2 == 4: base_score += 15; correct_count += 1
                        if a3 == 2: base_score += 15; correct_count += 1
                        if a4 == "기둥 B가 받는 힘이 더 크다.": base_score += 15; correct_count += 1
                        if a5 == "승용차 (무게중심이 낮고 지지면이 넓어 안정성이 높다.)": base_score += 20; correct_count += 1
                        if a6 == 2: base_score += 20; correct_count += 1
        ...
        if st.session_state.eval_status == "완료" and st.session_state.eval_results is not None:
            ...
            st.write(f"### 최종 환산 점수: {final_score}점")
